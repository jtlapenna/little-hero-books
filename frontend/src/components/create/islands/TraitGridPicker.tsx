  ...
  return (
    <fieldset className="trait-grid-picker" aria-label={`${name}: ${value ? `${value} selected` : 'select one'}`}>
      ...
      <style>{`
        ...
        .trait-grid-picker__grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md, 1rem); }
        @media (min-width: 640px) { .trait-grid-picker__grid { grid-template-columns: repeat(3, 1fr); } }
        .trait-grid-picker__option { position: relative; display: flex; flex-direction: column; align-items: center; padding: var(--spacing-md, 1rem); border: 2px solid rgba(45,49,66,0.15); border-radius: 14px; background: #fff; cursor: pointer; min-height: 200px; transition: border-color 0.15s ease, box-shadow 0.15s ease; }
