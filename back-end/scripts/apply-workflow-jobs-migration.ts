...
async function main() {
  ...
  try {
    ...
    await verifyTables();
    console.log('Applied workflow_jobs migration via exec_sql RPC.');
    return;
  } catch (error) {
    attempts.push({
      mode: 'exec_sql',
  ...
  try {
    ...
    await verifyTables();
    console.log('Applied workflow_jobs migration via Supabase Management API.');
    return;
  } catch (error) {
    attempts.push({
      mode: 'management_api',
  ...
  console.error('Failed to apply workflow_jobs migration automatically.');
...
main().catch((error) => {
  console.error('[apply-workflow-jobs-migration] Unexpected failure:', error);
  process.exit(1);
});
