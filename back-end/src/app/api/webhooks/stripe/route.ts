...
export async function POST(request: NextRequest) {
  ...
  const response = await withIdempotency(
    ...
    async () => {
      ...
      console.log('[Webhook Stripe] Confirmation email check: platform=%s, customer_email=%s', platform ?? 'undefined', customerEmail ? `${customerEmail.substring(0, 3)}...` : 'missing');
      if (platform !== 'd2c') {
        console.log('[Webhook Stripe] Skipping confirmation email: platform is not d2c');
