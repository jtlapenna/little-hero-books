...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    note: 'If AMAZON_PREVIEW_NOTIFICATIONS_ENABLED shows "NOT SET", you need to: 1) Set it in Vercel Project Settings → Environment Variables, 2) Make sure it\'s set for Production, 3) Trigger a new deployment (Redeploy)'
  });
}
