...
export function buildW3PreviewPlanResponse(
  body: JsonRecord,
): BuildW3PreviewPlanResult & { success: true } {
  return {
    success: true,
...
export async function POST(request: NextRequest) {
  ...
  try {
    return NextResponse.json(buildW3PreviewPlanResponse(body));
  } catch (error: unknown) {
    console.error('[Internal W3 Build Preview Plan] Error:', error);
    return NextResponse.json(
      {
        error: 'Failed to build W3 preview plan',
