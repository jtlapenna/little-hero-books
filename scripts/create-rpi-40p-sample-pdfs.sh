...
SPINE_PX=38           # 0.125 in at 300 dpi, rounded for 26-50 softcover pages
SPINE_COLOR="#4F5956"
...
tmp="$OUT_DIR/rpi-8x8-40p-pages"
rm -rf "$tmp"
mkdir -p "$tmp"

# RPI account SKU contains "_40_", so this builds a 40-interior-page sample.
pages=(
  00 01 02 03 04 05 06 07 08 09
  10 11 12 13 14 15 16 00 01 02
  03 04 05 06 07 08 09 10 11 12
  13 14 15 16 00 01 02 03 04 05
)
...
magick -density 300 "$tmp"/*-guts-*.png -compress JPEG -quality 95 "$OUT_DIR/rpi-8x8-40p-guts.pdf"
...
magick "$tmp/rpi-cover-spread.png" +repage -units PixelsPerInch -density 300 \
  -compress JPEG -quality 95 "$OUT_DIR/rpi-8x8-40p-cover.pdf"
...
} > "$OUT_DIR/rpi-8x8-40p-page-order.tsv"

magick montage "$tmp"/*-guts-*.png "$tmp/rpi-cover-spread.png" \
  -thumbnail 300x300 -tile 8x -geometry +12+12 "$OUT_DIR/rpi-8x8-40p-contact-sheet.jpg"

echo "Created:"
echo "$OUT_DIR/rpi-8x8-40p-cover.pdf"
echo "$OUT_DIR/rpi-8x8-40p-guts.pdf"
echo "$OUT_DIR/rpi-8x8-40p-contact-sheet.jpg"
