...
/**
 * Upload edge map pose reference images to Cloudflare R2
 * This replaces the grayscale pose images with pure edge maps
 */
...
const INPUT_DIR = path.join(__dirname, '../assets/poses/edges');
...
if (!ACCESS_KEY_ID || !SECRET_ACCESS_KEY) {
  ...
  console.error('   node scripts/upload-edge-maps-to-r2.js');
  process.exit(1);
}
...
async function main() {
  console.log('Uploading edge map pose images to R2...\n');
  ...
  console.log('\nNext steps:');
  console.log('1. Import the updated workflow JSON into n8n');
  console.log('2. Test the workflow - AI should now only see pose structure');
  console.log('3. Generated characters should match custom character exactly');
}
