# Follow-up template (max 1, after 10+ days of no response)
...
- Don't change the subject — that resets the conversation.
...
Update pipeline.md: status → `follow-up-sent`, last activity date → today.

If still no response after 10+ more days: mark status `no-response` and set `do-not-follow-up: true`. The creator may still respond later organically; if they do, treat them as a fresh conversation.

## Optional gentler variant (use only if Jeff requests)

```
Quick re-ping in case my last note got buried — totally fine to ignore if not a fit. Hope all's well.
