...
**Purpose:** Decide which issues to solve or confirm before duplicating n8n workflows for Book 2 (new assets, new book).  
**Created:** 2026-03-02
...
|---|--------|-------------------|
| **17** | [W3 concurrent orders – only one runs](17-w3-concurrent-orders-only-one-runs.md) | Throughput/concurrency will matter more with two books; duplicate workflows won’t help if W3 is single-threaded or drops the second order. |
...
| **29** | [W4 PDFMonkey half-rendered pages](_needs-review/29-w4-pdfmonkey-final-pdf-half-rendered-pages.md) | **Critical but in progress.** Layer 1b + auth done; Layer 2 planned. Prioritize before high volume (e.g. sibling aggregation); document current status and next step. |
...
3. **Fix reliability / data**  
   - 17 (W3 concurrency), 32 (manifest_2a_url), 10 (2B QA).

4. **Plan migration**  
   - 28 (Amazon v2026): scope and phases; implement before deadline.

5. **Then**  
   - Duplicate workflows for Book 2 (new assets, new book ID, same structure).
