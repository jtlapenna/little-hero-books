...
- `W4.1` now has the first operator-facing sibling recovery surface too, still without resuming live proofing:
  ...
  - backend deploy revisions [`07bd770d.little-hero-labs-admin.pages.dev`](https://07bd770d.little-hero-labs-admin.pages.dev) and [`f3be4c88.little-hero-labs-admin.pages.dev`](https://f3be4c88.little-hero-labs-admin.pages.dev) now serve the page/API and the grouped callback hardening, with token-auth smoke reads on both preview and custom domain returning zero current candidates
  - this pass stayed non-live and non-billable: no `W4.1` replay was triggered, and validation stopped at test/build/deploy smoke coverage only
