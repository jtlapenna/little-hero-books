...
- `W4.1` now also has the first grouped paid-cutover slice, but still no paid submit path:
  ...
  - grouped sibling submit builder [`w4-sibling-print-input.ts`](/Users/jeff/Projects/little-hero-books/back-end/src/lib/books/w4-sibling-print-input.ts) now supports grouped production intent, grouped production dry-run, grouped approval-token verification, and structured grouped production guard results
