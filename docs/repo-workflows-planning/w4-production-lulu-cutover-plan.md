...
- Latest March 29, 2026 shipping-cutover checkpoint:
  - repo helper [`w4-submit-input.ts`](/Users/jeff/Projects/little-hero-books/back-end/src/lib/books/w4-submit-input.ts) now maps Amazon-style `STANDARD` shipping to Lulu `MAIL` instead of `GROUND`, while still preserving explicit ground requests
  - focused regression coverage in [`test-w4-submit-input.ts`](/Users/jeff/Projects/little-hero-books/back-end/scripts/test-w4-submit-input.ts) now locks both `STANDARD -> MAIL` and explicit `GROUND_HOME -> GROUND`
  - fresh candidate `441-0329202-9000006` proved the shipping fix in live admin preflight: `/api/admin/w4-production` returned `safeForProductionPilot = true`, `productionGuard.reason = "production_ready"`, and `shippingLevelSent = "MAIL"`
  - the first retry attempt on `441-0329202-9000006` remained non-production because the env gate was turned back off immediately after webhook acceptance; admin workflow telemetry recorded `workflow_jobs.id = 174` finishing `succeeded` with `submitMode = "skip"` / `externalProvider = "lulu-sandbox"`, which confirmed the operational rule that the env gate must stay enabled until the async workflow actually crosses the submit seam
